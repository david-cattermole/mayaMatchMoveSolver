...
int test_once_3de_anamorphic_std_deg4_rescaled() {
    const auto test_name = "test_once_3de_anamorphic_std_deg4_rescaled";
    const auto identity_prefix =
        "test_once_3de_anamorphic_std_deg4_rescaled: start";
    const auto undistort_prefix =
        "test_once_3de_anamorphic_std_deg4_rescaled: undistort";
    const auto undistort_compare = " -> ";
    const auto redistort_prefix =
        "test_once_3de_anamorphic_std_deg4_rescaled: redistort";
    const auto redistort_compare = " -> ";
    const auto print_prefix =
        "test_once_3de_anamorphic_std_deg4_rescaled: output";
    ...
    const double rescaled = 2.0;

    auto lens = mmlens::LensModel3deAnamorphicDeg4RotateSqueezeXYRescaled();
    ...
    apply_distortion_loop<
        double, double,
        mmlens::LensModel3deAnamorphicDeg4RotateSqueezeXYRescaled,
        kDirectionUndistort>(width, height, stride, stride, in_data, temp_data,
                             lens);
    if (do_print) {
    ...
    apply_distortion_loop<
        double, double,
        mmlens::LensModel3deAnamorphicDeg4RotateSqueezeXYRescaled,
        kDirectionRedistort>(width, height, stride, stride, temp_data, out_data,
                             lens);
    if (do_print) {
