...
use crate::nuke_lens_common::lookup_lens_model_type;

// Re-export all public items from common so existing consumers
// of nuke_lens_reader continue to work unchanged.
pub use crate::nuke_lens_common::CameraParameters;
pub use crate::nuke_lens_common::FrameNumber;
pub use crate::nuke_lens_common::LayerSize;
pub use crate::nuke_lens_common::LensModelType;
pub use crate::nuke_lens_common::NukeLensData;
pub use crate::nuke_lens_common::ParameterBlock;
pub use crate::nuke_lens_common::MAX_LENS_PARAMETER_COUNT;
pub use crate::nuke_lens_common::NUKE_3DE4_ANAMORPHIC_DEG6;
pub use crate::nuke_lens_common::NUKE_3DE4_ANAMORPHIC_STD_DEG4;
pub use crate::nuke_lens_common::NUKE_3DE4_ANAMORPHIC_STD_DEG4_RESCALED;
pub use crate::nuke_lens_common::NUKE_3DE4_ANAMORPHIC_STD_DEG6;
pub use crate::nuke_lens_common::NUKE_3DE4_ANAMORPHIC_STD_DEG6_RESCALED;
pub use crate::nuke_lens_common::NUKE_3DE4_CLASSIC;
pub use crate::nuke_lens_common::NUKE_3DE4_RADIAL_STD_DEG4;
pub use crate::nuke_lens_common::STATIC_FRAME_NUMBER;
...
#[derive(Clone, Debug)]
struct Node {
    pub lens_model_type: LensModelType,
...
impl Node {
    fn new() -> Node {
        Node {
            lens_model_type: LensModelType::Uninitialized,
            node_name: "".to_string(),
            node_type: "".to_string(),
            knobs: HashMap::new(),
...
fn parse_nuke_file_lines(lines: Vec<String>) -> Result<NukeLensData> {
            ...
            LensModelType::TdeClassic => {
                for frame_number in min_frame..=max_frame {
                    ...
                    let mut parameter_block =
                        [0.0_f64; MAX_LENS_PARAMETER_COUNT];
...
/// Read a Nuke file, stripping comments and trimming whitespace.
pub fn read_nuke_file_lines(file_path: &str) -> Vec<String> {
...
/// Read and parse a Nuke .nk lens distortion file.
pub fn read_nuke_lens_file(file_path: &str) -> Result<NukeLensData> {
    let lines = read_nuke_file_lines(file_path);
    parse_nuke_file_lines(lines)
}
...
mod tests {
    ...
    fn test_data_dir() -> std::path::PathBuf {
        ...
        // Go up from lib/rust/mmio to project root.
        path.pop();
        path.pop();
        path.pop();
        path.push("tests");
    ...
    fn check_lens_file(file_name: &str, expect_layers: bool) {
        let path = test_data_dir().join(file_name);
        if !path.exists() {
            // Skip if test data not available.
            return;
        }
        let data = read_nuke_lens_file(path.to_str().unwrap()).unwrap();
        if expect_layers {
            assert!(
                data.layer_count > 0,
                "Expected non-zero layer count for {}",
                file_name
            );
        }
        assert_eq!(
            data.layer_lens_model_types.len(),
            data.layer_count as usize
        );
        assert_eq!(data.layer_frame_range.len(), data.layer_count as usize);
    }

    #[test]
    fn test_ldpk_classic_v003() {
        check_lens_file("ldpk_nuke_distortion_classic_v003.nk", true);
    }

    #[test]
    fn test_ldpk_classic_v004() {
        check_lens_file("ldpk_nuke_distortion_classic_v004.nk", true);
    }

    #[test]
    fn test_ldpk_anamorphic6_v003() {
        check_lens_file("ldpk_nuke_distortion_anamorphic6_v003.nk", false);
    }

    #[test]
    fn test_ldpk_v001() {
        check_lens_file("ldpk_nuke_distortion_v001.nk", true);
    }

    #[test]
    fn test_ldpk_v002() {
        check_lens_file("ldpk_nuke_distortion_v002.nk", true);
    }

    #[test]
    fn test_weta_v001() {
        check_lens_file("weta_nuke_distortion_v001.nk", false);
    }

    #[test]
    fn test_weta_v002() {
        check_lens_file("weta_nuke_distortion_v002.nk", false);
    }
}
