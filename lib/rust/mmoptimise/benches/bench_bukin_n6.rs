...
//! Bukin N6 function benchmarks across all three solvers.
//!
//! The Bukin N6 function has a narrow valley with a discontinuous
//! derivative at the global minimum. It's particularly challenging
//! for gradient-based optimizers and tests numerical stability.
...
fn bench_bukin_n6_solver_comparison(c: &mut Criterion) {
    let mut group = c.benchmark_group("bukin_n6_solver_comparison");
...
fn bench_bukin_n6_config_comparison(c: &mut Criterion) {
    let mut group = c.benchmark_group("bukin_n6_config_comparison");
...
criterion_group!(
    benches,
    bench_bukin_n6_solver_comparison,
    bench_bukin_n6_config_comparison
);
