...
/// Benchmark using LevenbergMarquardtWorkspace::new() for each iteration (baseline)
fn bench_rosenbrock_memory_comparison_new(c: &mut Criterion) {
    ...
    group.bench_function("using_new", |b| {
        b.iter(|| {
            for start_point in &starting_points {
                black_box(
                    run_lm_benchmark(&problem, default_config, start_point)
                        .unwrap(),
                );
            }
        })
...
/// Benchmark using LevenbergMarquardtWorkspace::reuse_with() (optimized)
