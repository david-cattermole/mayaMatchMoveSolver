...
//! Integration test for complete camera solve using cube2 B dataset.
...
fn run_camera_solve_cube2_b(
    ...
    let (dataset_name, condition) = if is_noisy {
        ...
        (
            format!("Cube2 B (noisy, {})", solver_suffix),
            DataCondition::Noisy,
        )
    } else {
        (
            format!("Cube2 B (clean, {})", solver_suffix),
            DataCondition::Clean,
        )
    };
...
fn test_camera_solve_clean_cube2_b_dense_lm() -> Result<()> {
    run_camera_solve_cube2_b(false, BundleAdjustmentSolverType::DenseLM)
}

#[test]
fn test_camera_solve_clean_cube2_b_sparse_lm() -> Result<()> {
    run_camera_solve_cube2_b(
        false,
...
#[test]
#[ignore] // TODO: Make this run faster so we can test faster.
fn test_camera_solve_noisy_cube2_b_dense_lm() -> Result<()> {
    run_camera_solve_cube2_b(true, BundleAdjustmentSolverType::DenseLM)
}

#[test]
#[ignore]
fn test_camera_solve_noisy_cube2_b_sparse_lm() -> Result<()> {
    run_camera_solve_cube2_b(
        true,
        BundleAdjustmentSolverType::SparseLevenbergMarquardt,
    )
