...
//! Integration test for complete camera solve using cube2 D dataset.
...
fn run_camera_solve_cube2_d(
    is_noisy: bool,
    solver_type: BundleAdjustmentSolverType,
) -> Result<()> {
    let (mut markers, mut frame_range) =
        load_marker_data("test_cube2_d_markers_v1_fmt.uv")?;
    ...
    let (dataset_name, condition) = if is_noisy {
        ...
        (
            format!("Cube2 D (noisy, {})", solver_suffix),
            DataCondition::Noisy,
        )
    } else {
        (
            format!("Cube2 D (clean, {})", solver_suffix),
            DataCondition::Clean,
        )
    };
...
fn test_camera_solve_clean_cube2_d_dense_lm() -> Result<()> {
    run_camera_solve_cube2_d(false, BundleAdjustmentSolverType::DenseLM)
}

#[test]
fn test_camera_solve_clean_cube2_d_sparse_lm() -> Result<()> {
    run_camera_solve_cube2_d(
        false,
...
#[test]
#[ignore] // TODO: Make this run faster so we can test faster.
fn test_camera_solve_noisy_cube2_d_dense_lm() -> Result<()> {
    run_camera_solve_cube2_d(true, BundleAdjustmentSolverType::DenseLM)
}

#[test]
#[ignore]
fn test_camera_solve_noisy_cube2_d_sparse_levenberg_marquardt() -> Result<()> {
    run_camera_solve_cube2_d(
        true,
        BundleAdjustmentSolverType::SparseLevenbergMarquardt,
    )
