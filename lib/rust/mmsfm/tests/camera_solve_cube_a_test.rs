...
//! Integration test for complete camera solve using cube A dataset.
...
fn run_camera_solve_cube_a(
    is_noisy: bool,
    solver_type: BundleAdjustmentSolverType,
) -> Result<()> {
    let (mut markers, mut frame_range) =
        load_marker_data("test_cube_a_markers_v1_fmt.uv")?;
    // frame_range.end_frame = 51;
    ...
    let (dataset_name, condition) = if is_noisy {
        add_noise_to_markers(&mut markers, 0.002);
        (
            format!("Cube A (noisy, {})", solver_suffix),
            DataCondition::Noisy,
        )
    } else {
        (
            format!("Cube A (clean, {})", solver_suffix),
            DataCondition::Clean,
        )
    };
...
fn test_camera_solve_clean_cube_a_dense_lm() -> Result<()> {
    run_camera_solve_cube_a(false, BundleAdjustmentSolverType::DenseLM)
}

#[test]
fn test_camera_solve_clean_cube_a_sparse_lm() -> Result<()> {
    run_camera_solve_cube_a(
        false,
...
#[test]
#[ignore] // TODO: Make this run faster so we can test faster.
fn test_camera_solve_noisy_cube_a_dense_lm() -> Result<()> {
    run_camera_solve_cube_a(true, BundleAdjustmentSolverType::DenseLM)
}

#[test]
#[ignore]
fn test_camera_solve_noisy_cube_a_sparse_lm() -> Result<()> {
    run_camera_solve_cube_a(
        true,
        BundleAdjustmentSolverType::SparseLevenbergMarquardt,
    )
