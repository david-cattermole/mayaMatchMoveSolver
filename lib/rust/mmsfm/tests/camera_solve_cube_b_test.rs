...
//! Integration test for complete camera solve using cube B dataset.
...
fn run_camera_solve_cube_b(
    ...
    let (dataset_name, condition) = if is_noisy {
        ...
        (
            format!("Cube B (noisy, {})", solver_suffix),
            DataCondition::Noisy,
        )
    } else {
        (
            format!("Cube B (clean, {})", solver_suffix),
            DataCondition::Clean,
        )
    };
...
fn test_camera_solve_clean_cube_b_dense_lm() -> Result<()> {
    run_camera_solve_cube_b(false, BundleAdjustmentSolverType::DenseLM)
}

#[test]
fn test_camera_solve_clean_cube_b_sparse_lm() -> Result<()> {
    run_camera_solve_cube_b(
        false,
...
#[test]
#[ignore] // TODO: Make this run faster so we can test faster.
fn test_camera_solve_noisy_cube_b_dense_lm() -> Result<()> {
    run_camera_solve_cube_b(true, BundleAdjustmentSolverType::DenseLM)
}

#[test]
#[ignore] // TODO: This test fails - fix it.
fn test_camera_solve_noisy_cube_b_sparse_lm() -> Result<()> {
    run_camera_solve_cube_b(
        true,
        BundleAdjustmentSolverType::SparseLevenbergMarquardt,
    )
