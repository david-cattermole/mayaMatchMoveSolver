...
//! Integration test for complete camera solve using cube C dataset.
...
fn run_camera_solve_cube_c(
    ...
    let (dataset_name, condition) = if is_noisy {
        ...
        (
            format!("Cube C (noisy, {})", solver_suffix),
            DataCondition::Noisy,
        )
    } else {
        (
            format!("Cube C (clean, {})", solver_suffix),
            DataCondition::Clean,
        )
    };
...
fn test_camera_solve_clean_cube_c_dense_lm() -> Result<()> {
    run_camera_solve_cube_c(false, BundleAdjustmentSolverType::DenseLM)
}

#[test]
fn test_camera_solve_clean_cube_c_sparse_lm() -> Result<()> {
    run_camera_solve_cube_c(
        false,
...
#[test]
#[ignore] // TODO: Make this run faster so we can test faster.
fn test_camera_solve_noisy_cube_c_dense_lm() -> Result<()> {
    run_camera_solve_cube_c(true, BundleAdjustmentSolverType::DenseLM)
}

#[test]
#[ignore] // TODO: This test fails - fix it.
fn test_camera_solve_noisy_cube_c_sparse_lm() -> Result<()> {
    run_camera_solve_cube_c(
        true,
        BundleAdjustmentSolverType::SparseLevenbergMarquardt,
    )
