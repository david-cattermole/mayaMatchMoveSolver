...
//! Integration test for complete camera solve using HCW Painting dataset.
...
fn run_camera_solve_hcw_painting(
    is_noisy: bool,
    ...
    let (dataset_name, condition) = if is_noisy {
        ...
        (
            format!("HCW Painting (noisy, {})", solver_suffix),
            DataCondition::Noisy,
        )
    } else {
        (
            format!("HCW Painting (clean, {})", solver_suffix),
            DataCondition::Clean,
        )
    };
    ...
    let initial_focal_length = MillimeterUnit::new(20.0);
...
#[test]
#[ignore] // Slow
fn test_camera_solve_clean_hcw_painting_dense_lm() -> Result<()> {
    run_camera_solve_hcw_painting(false, BundleAdjustmentSolverType::DenseLM)
}

#[test]
fn test_camera_solve_clean_hcw_painting_sparse_lm() -> Result<()> {
    run_camera_solve_hcw_painting(
        false,
...
#[test]
#[ignore] // Slow
fn test_camera_solve_noisy_hcw_painting_dense_lm() -> Result<()> {
    run_camera_solve_hcw_painting(true, BundleAdjustmentSolverType::DenseLM)
}

#[test]
#[ignore]
fn test_camera_solve_noisy_hcw_painting_sparse_lm() -> Result<()> {
    run_camera_solve_hcw_painting(
        true,
        BundleAdjustmentSolverType::SparseLevenbergMarquardt,
    )
