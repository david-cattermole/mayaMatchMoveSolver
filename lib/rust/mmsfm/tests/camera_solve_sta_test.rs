...
//! Integration test for complete camera solve using stA dataset.
...
fn run_camera_solve_sta(
    is_noisy: bool,
    solver_type: BundleAdjustmentSolverType,
) -> Result<()> {
    let (mut markers, mut frame_range) = load_marker_data("stA_v1_format.uv")?;
    ...
    let (dataset_name, condition) = if is_noisy {
        ...
        (
            format!("stA (noisy, {})", solver_suffix),
            DataCondition::Noisy,
        )
    } else {
        (
            format!("stA (clean, {})", solver_suffix),
            DataCondition::Clean,
        )
    };
    ...
    let initial_focal_length = MillimeterUnit::new(41.6);
...
fn test_camera_solve_clean_sta_dense_lm() -> Result<()> {
    run_camera_solve_sta(false, BundleAdjustmentSolverType::DenseLM)
}

#[test]
fn test_camera_solve_clean_sta_sparse_lm() -> Result<()> {
    run_camera_solve_sta(
        false,
...
#[test]
#[ignore]
fn test_camera_solve_noisy_sta_sparse_lm() -> Result<()> {
    run_camera_solve_sta(
        true,
        BundleAdjustmentSolverType::SparseLevenbergMarquardt,
    )
