...
//! Relative pose tests for the Cube2 A dataset.
...
fn run_relative_pose_cube2_a(is_noisy: bool) -> Result<()> {
    ...
    let (dataset_name, condition) = if is_noisy {
        ...
        ("Cube2 A (noisy)", DataCondition::Noisy)
    } else {
        ("Cube2 A (clean)", DataCondition::Clean)
    };
...
#[test]
fn test_relative_pose_clean_cube2_a() -> Result<()> {
    run_relative_pose_cube2_a(false)
}

#[test]
fn test_relative_pose_noisy_cube2_a() -> Result<()> {
    run_relative_pose_cube2_a(true)
}
