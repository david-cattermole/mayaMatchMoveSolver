...
//! Relative pose tests for the Cube2 B dataset.
...
fn run_relative_pose_cube2_b(is_noisy: bool) -> Result<()> {
    ...
    let (dataset_name, condition) = if is_noisy {
        ...
        ("Cube2 B (noisy)", DataCondition::Noisy)
    } else {
        ("Cube2 B (clean)", DataCondition::Clean)
    };
...
#[test]
fn test_relative_pose_clean_cube2_b() -> Result<()> {
    run_relative_pose_cube2_b(false)
}

#[test]
fn test_relative_pose_noisy_cube2_b() -> Result<()> {
    run_relative_pose_cube2_b(true)
}
