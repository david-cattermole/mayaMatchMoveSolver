...
//! Relative pose tests for the Cube2 D dataset.
...
fn run_relative_pose_cube2_d(is_noisy: bool) -> Result<()> {
    ...
    let (dataset_name, condition) = if is_noisy {
        ...
        ("Cube2 D (noisy)", DataCondition::Noisy)
    } else {
        ("Cube2 D (clean)", DataCondition::Clean)
    };
...
#[test]
fn test_relative_pose_clean_cube2_d() -> Result<()> {
    run_relative_pose_cube2_d(false)
}

#[test]
fn test_relative_pose_noisy_cube2_d() -> Result<()> {
    run_relative_pose_cube2_d(true)
}
