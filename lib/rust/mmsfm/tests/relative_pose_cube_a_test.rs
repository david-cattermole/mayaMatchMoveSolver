...
//! Relative pose tests for the Cube A dataset.
...
fn run_relative_pose_cube_a(is_noisy: bool) -> Result<()> {
    ...
    let (dataset_name, condition) = if is_noisy {
        ...
        ("Cube A (noisy)", DataCondition::Noisy)
    } else {
        ("Cube A (clean)", DataCondition::Clean)
    };
...
#[test]
fn test_relative_pose_clean_cube_a() -> Result<()> {
    run_relative_pose_cube_a(false)
}

#[test]
fn test_relative_pose_noisy_cube_a() -> Result<()> {
    run_relative_pose_cube_a(true)
}
