...
//! Relative pose tests for the Cube B dataset.
...
fn run_relative_pose_cube_b(is_noisy: bool) -> Result<()> {
    ...
    let (dataset_name, condition) = if is_noisy {
        ...
        ("Cube B (noisy)", DataCondition::Noisy)
    } else {
        ("Cube B (clean)", DataCondition::Clean)
    };
...
#[test]
fn test_relative_pose_clean_cube_b() -> Result<()> {
    run_relative_pose_cube_b(false)
}

#[test]
fn test_relative_pose_noisy_cube_b() -> Result<()> {
    run_relative_pose_cube_b(true)
}
