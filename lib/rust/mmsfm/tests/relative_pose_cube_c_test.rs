...
//! Relative pose tests for the Cube C dataset.
...
use common::{
    ...
    visualization::scene::ViewConfigurationBuilder, FramePair,
};
use mmsfm_rust::datatype::{
    CameraFilmBack, ImageSize, MillimeterUnit, UnitValue,
};

fn run_relative_pose_cube_c(is_noisy: bool) -> Result<()> {
    ...
    let (dataset_name, condition) = if is_noisy {
        ...
        ("Cube C (noisy)", DataCondition::Noisy)
    } else {
        ("Cube C (clean)", DataCondition::Clean)
    };
...
#[test]
fn test_relative_pose_clean_cube_c() -> Result<()> {
    run_relative_pose_cube_c(false)
}

#[test]
fn test_relative_pose_noisy_cube_c() -> Result<()> {
    run_relative_pose_cube_c(true)
}
