...
//! Triangulation test with camera B below, tilted up 15 degrees.
...
#[test]
fn triangulation_d_test() -> Result<()> {
    let config = CameraConfiguration::new_tilt(
        Point3::new(0.0, -5.0, 0.0), // 5 units down.
        15.0,                         // 15 degrees up tilt.
    );
