...
fn run_triangulation_cube2_a(is_noisy: bool) -> Result<()> {
    ...
    let (test_base_name, condition, threshold) = if is_noisy {
        ...
        ("Triangulation Cube2 - A (noisy)", DataCondition::Noisy, 2.0)
    } else {
        ("Triangulation Cube2 - A (clean)", DataCondition::Clean, 1.0)
    };
...
#[test]
fn test_triangulation_clean_cube2_a() -> Result<()> {
    run_triangulation_cube2_a(false)
}

#[test]
fn test_triangulation_noisy_cube2_a() -> Result<()> {
    run_triangulation_cube2_a(true)
}
