...
use crate::common::{
    add_noise_to_markers, load_marker_data, run_triangulation_test_from_file,
};
use crate::common::{
...
fn run_triangulation_cube2_b(is_noisy: bool) -> Result<()> {
    ...
    let (test_base_name, condition, threshold) = if is_noisy {
        ...
        ("Triangulation Cube2 - B (noisy)", DataCondition::Noisy, 5.0)
    } else {
        ("Triangulation Cube2 - B (clean)", DataCondition::Clean, 1.0)
    };
...
#[test]
fn test_triangulation_clean_cube2_b() -> Result<()> {
    run_triangulation_cube2_b(false)
}

#[test]
fn test_triangulation_noisy_cube2_b() -> Result<()> {
    run_triangulation_cube2_b(true)
}
