...
fn run_triangulation_cube2_c(is_noisy: bool) -> Result<()> {
    ...
    let (test_base_name, condition) = if is_noisy {
        ...
        ("Triangulation Cube2 - C (noisy)", DataCondition::Noisy)
    } else {
        ("Triangulation Cube2 - C (clean)", DataCondition::Clean)
    };
...
#[test]
fn test_triangulation_clean_cube2_c() -> Result<()> {
    run_triangulation_cube2_c(false)
}

#[test]
fn test_triangulation_noisy_cube2_c() -> Result<()> {
    run_triangulation_cube2_c(true)
}
