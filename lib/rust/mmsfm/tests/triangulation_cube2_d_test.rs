...
fn run_triangulation_cube2_d(is_noisy: bool) -> Result<()> {
    ...
    let (test_base_name, condition) = if is_noisy {
        ...
        ("Triangulation Cube2 - D (noisy)", DataCondition::Noisy)
    } else {
        ("Triangulation Cube2 - D (clean)", DataCondition::Clean)
    };
...
#[test]
fn test_triangulation_clean_cube2_d() -> Result<()> {
    run_triangulation_cube2_d(false)
}

#[test]
fn test_triangulation_noisy_cube2_d() -> Result<()> {
    run_triangulation_cube2_d(true)
}
