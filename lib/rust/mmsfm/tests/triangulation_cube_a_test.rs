...
fn run_triangulation_cube_a(is_noisy: bool) -> Result<()> {
    ...
    let (test_base_name, condition, threshold) = if is_noisy {
        ...
        ("Triangulation Cube - A (noisy)", DataCondition::Noisy, 2.0)
    } else {
        ("Triangulation Cube - A (clean)", DataCondition::Clean, 1.0)
    };
...
#[test]
fn test_triangulation_clean_cube_a() -> Result<()> {
    run_triangulation_cube_a(false)
}

#[test]
fn test_triangulation_noisy_cube_a() -> Result<()> {
    run_triangulation_cube_a(true)
}
