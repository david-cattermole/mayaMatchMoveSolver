...
fn run_triangulation_cube_c(is_noisy: bool) -> Result<()> {
    ...
    let (test_base_name, condition) = if is_noisy {
        ...
        ("Triangulation Cube - C (noisy)", DataCondition::Noisy)
    } else {
        ("Triangulation Cube - C (clean)", DataCondition::Clean)
    };
...
#[test]
fn test_triangulation_clean_cube_c() -> Result<()> {
    run_triangulation_cube_c(false)
}

#[test]
fn test_triangulation_noisy_cube_c() -> Result<()> {
    run_triangulation_cube_c(true)
}
