...
def _world_bake(pivot, main, loc_grp, start, end,
                smart_bake=False,
                dynamic_pivot=False):
...
def _create_controller_world_space(name,
    ...
    if current_frame is True:
        ...
        maya.cmds.cutKey(main_driver_loc,
...
def _create_controller_object_space(name,
    ...
    if current_frame is True:
        ...
        inner_start_frame, inner_end_frame = \
            time_utils.get_maya_timeline_range_inner()
        maya.cmds.cutKey(main_driver_loc,
                         time=(inner_start_frame, inner_end_frame))
        maya.cmds.setKeyframe(main_driver_loc)
    maya.cmds.parentConstraint(
...
def _create_controller_screen_space(name,
    ...
    aim_con = maya.cmds.aimConstraint(camera, screen_loc,
    ...
    # Current frame
    if current_frame is True:
        maya.cmds.setKeyframe(screen_loc, zero_loc,
                              time=(start_frame, start_frame))
        maya.cmds.cutKey(loc_grp_node, screen_loc, zero_loc,
...
def remove_controller(controller_node, frame_start, frame_end,
    ...
    for constraint in constraints:
        ...
        driven_nodes += maya.cmds.listConnections(
            attr,
            destination=False,
            source=True) or []
