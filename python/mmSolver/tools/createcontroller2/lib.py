...
def _worldBake(pivot, main, loc_grp, start, end, smart=False):
    ...
    #bake attributes 
    fastbake_lib.bakeAttributes(loc_grp, start, end, smart, False, [])
...
def createController(name,
    ...
    if space == "screenSpace":
        ...
        #bake attributes       
        fastbake_lib.bakeAttributes(screen_loc, start_frame, end_frame, False, from_channel_box, attrs)        
...
def removeController(controllerNode):
    ...
    if driven_node:
        #bake attributes
        fastbake_lib.bakeAttributes(driven_node, frame_start, frame_end, True, True, attrs)
