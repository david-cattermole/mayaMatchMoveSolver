...
def _world_bake(pivot, main, loc_grp, start, end, smart_bake=False,
    ...
    if "vtx" in pivot:
        ...
        if dynamic_pivot:
            con = cmds.pointConstraint(pivot, loc_grp, maintainOffset=False)            
        if not dynamic_pivot:
            ...
            con = cmds.parentConstraint(main, loc_grp, maintainOffset=True)                       
        fastbake_lib.bake_attributes(loc_grp, attrs, start, end,
...
def _create_controller_world_space(name,
    ...
    loc_grp_node = _world_bake(
        ...
        start_frame, end_frame, smart_bake=smart_bake, dynamic_pivot=dynamic_pivot)
    ...
    if current_frame is True:
        ...
        cmds.cutKey(main_driver_loc, time=(inner_start_frame, inner_end_frame))
        cmds.setKeyframe(main_driver_loc)
...
def _create_controller_object_space(name,
    ...
    loc_grp_node = _world_bake(
        pivot_node, main_node, loc_grp_node,
        start_frame, end_frame, smart_bake=False, dynamic_pivot=dynamic_pivot)
...
def _create_controller_screen_space(name,
                                    ...
                                    current_frame, camera, dynamic_pivot =True):
    skip_translate_attr = _skip_translate_attributes(main_node)
