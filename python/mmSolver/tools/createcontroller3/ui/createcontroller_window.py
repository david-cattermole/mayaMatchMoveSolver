...
import mmSolver.tools.createcontroller3.ui.createcontroller_layout as createcontroller_layout
...
class ComboBox(QtWidgets.QComboBox):
    ...
    def index_changed(self):
        ...
        if column == const.COLUMN_BAKE_INDEX and self.currentText() == const.BAKE_ITEM_SMART_BAKE:
            cell_widget = self.table_widget.cellWidget(row,
        ...
        elif column == const.COLUMN_SPACE_INDEX:
            camera_cell_widget = self.table_widget.cellWidget(row,
                                                      const.COLUMN_CAMERA_INDEX)
            if self.currentText() == const.SPACE_ITEM_WORLD_SPACE:
                camera_cell_widget.setEnabled(False) 
                camera_cell_widget.setCurrentIndex(0)          
                cell_widget = self.table_widget.cellWidget(row,
                                                        const.COLUMN_BAKE_INDEX)
                item_text = cell_widget.currentText()
...
class CreateControllerWindow(BaseWindow):
    ...
    def add_objects_clicked(self):
        ...
        main_names = self.form.get_existing_names_from_ui(
                                             const.COLUMN_MAIN_OBJECT_INDEX)
        ...
        if not has_constraints:
            for node in selection:
                if node not in main_names:
                    ...
                    object_indices = [const.COLUMN_CONTROLLER_NAME_INDEX,
                                    const.COLUMN_MAIN_OBJECT_INDEX,
                                    const.COLUMN_PIVOT_OBJECT_INDEX]                
                    for count, idx in enumerate(object_indices):
                        self.insert_item(current_row_count, object_indices[count],
                                        node)     
                    combo_indices = [const.COLUMN_PIVOT_TYPE_INDEX,
                                    const.COLUMN_BAKE_INDEX,
                                    const.COLUMN_SPACE_INDEX]
                    combo_items = [const.PIVOT_TYPE_ITEM_LIST,
                                const.BAKE_ITEM_LIST,
                                const.SPACE_ITEM_LIST]                
                    for count, idx in enumerate(combo_indices):
                        self.insert_combo_box(current_row_count,
                                            combo_indices[count],
                                            combo_items[count],
                                            True)
                        self.insert_combo_box(current_row_count,
    ...
    def resize_columns(self, win_width):
        ...
        win_width = win_width - (const.COLUMN_PIVOT_TYPE_INDEX_SIZE+
                                 const.COLUMN_BAKE_INDEX_SIZE+
                                 const.COLUMN_SPACE_INDEX_SIZE)        
        each_column_width = win_width / 4 
        index_list = const.COLUMN_INDICES_LIST
        column_size_list = [each_column_width,
                            each_column_width,
                            each_column_width,
                            each_column_width,
                            const.COLUMN_PIVOT_TYPE_INDEX_SIZE,
                            const.COLUMN_BAKE_INDEX_SIZE,
                            const.COLUMN_SPACE_INDEX_SIZE]        
        for count, idx in enumerate(column_size_list):
            self.form.mainTableWidget.horizontalHeader().resizeSection(
                                                        index_list[count],
                                                        column_size_list[count])
