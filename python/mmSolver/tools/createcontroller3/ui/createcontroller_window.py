...
class ComboBox(QtWidgets.QComboBox):
    ...
    def index_changed(self):
        ...
        if (column == ui_const.COLUMN_BAKE_INDEX
...
class CreateControllerWindow(BaseWindow):
    ...
    def insert_combo_box(self, row, column, item_list, enable_flag=True):
        combo_widget = ComboBox(row, column, item_list,
                                self.form.mainTableWidget)
            ...
            for node in selection:
                if node not in main_names:
                    ...
                    for count, idx in enumerate(object_indices):
                        self.insert_item(current_row_count,
                                         object_indices[count],
                                         node)
                    combo_indices = [
    ...
    def resize_columns(self, win_width):
        ...
        win_width = win_width - (ui_const.COLUMN_PIVOT_TYPE_INDEX_SIZE +
