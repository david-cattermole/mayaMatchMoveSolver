...
class Delaunator():
    ...
    def update(self, coords):
        ...
        for i in range(0, n):
            ...
            r = circumradius(i0x, i0y, i1x, i1y, coords[2 * i],
                             coords[2 * i + 1])
        ...
        for i in range(0, n):
            self._dists[i] = dist(coords[2 * i], coords[2 * i + 1], center[0],
                                  center[1])
        ...
        for k in range(0, len(self._ids)):
            ...
            if (k > 0 and abs(x - xp) <= EPSILON and abs(
                    y - yp) <= EPSILON): continue
            ...
            while True:
                q = self.hullNext[e]
                if orient(x, y, coords[2 * e], coords[2 * e + 1], coords[2 * q],
                          coords[2 * q + 1]): break
                e = q
            ...
            t = self._addTriangle(e, i, self.hullNext[e], -1, -1,
                                  self.hullTri[e])
            ...
            while True:
                q = self.hullNext[n]
                if not (
                        orient(x, y, coords[2 * n], coords[2 * n + 1],
                               coords[2 * q],
                               coords[2 * q + 1])): break
                t = self._addTriangle(n, i, q, self.hullTri[i], -1,
                                      self.hullTri[n])
                self.hullTri[i] = self._legalize(t + 2, coords)
            ...
            if (e == start):
                while True:
                    q = self.hullPrev[e]
                    if not (orient(x, y, coords[2 * q], coords[2 * q + 1],
                                   coords[2 * e], coords[2 * e + 1])): break
                    t = self._addTriangle(q, i, e, -1, self.hullTri[e],
                                          self.hullTri[q])
