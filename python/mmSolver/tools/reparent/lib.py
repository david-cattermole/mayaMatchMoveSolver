...
def reparent(children, parent,
    ...
    keyframe_times = keytime_utils.KeyframeTimes()
    keyframe_times.add_nodes_attrs(children_nodes, TFM_ATTRS, start_frame, end_frame)

    # Query current transforms
    tfm_cache = tfm_utils.TransformMatrixCache()
    for tfm_node, node in zip(children, children_nodes):
        frame_range = keyframe_times.get_times(node, sparse)
    ...
    for tfm_node in changed_tfm_nodes:
        node = tfm_node.get_node()
        times = keyframe_times.get_times(node, sparse)
