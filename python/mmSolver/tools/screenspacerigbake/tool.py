...
TRANSFORM_ATTR_LIST = ['tx', 'ty', 'tz', 'rx', 'ry', 'rz', 'sx', 'sy', 'sz']
...
def setZDepthKeyframes(node, start_frame, end_frame, values, anim_layer_name=None):
    kwargs = {}
    ...
    for i, frame in enumerate(frames):
        ...
        cmds.setKeyframe(
            node, attribute=SCREEN_Z_DEPTH_ATTR_NAME, value=values[i], **kwargs
        )
    return
...
class ScreenSpaceRigWindow(BaseWindow):
    def __init__(self, parent=None, name=None):
        super(ScreenSpaceRigWindow, self).__init__(parent, name=name)
...
class ScreenSpaceRigLayout(QtWidgets.QWidget):
    ...
    def setupUi(self, parent):
        ...
        spacerItem = QtWidgets.QSpacerItem(
            5, 17, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum
        )
        ...
        spacerItem2 = QtWidgets.QSpacerItem(
            5, 10, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum
        )
    ...
    def createConnections(self):
        ...
        self.select_action.triggered.connect(self.selectInOutliner)
    ...
    def createRig(self, camera, object, dlist, name, rigName):
        try:
            ...
            with ctx:
                ...
                cmds.addAttr(
                    main_grp,
                    ln=ATTRIBUTE_IDENTIFIER_NAME,
                    nn=ATTRIBUTE_IDENTIFIER_NICE_NAME,
                    dt='string',
                    k=False,
                )
                cmds.setAttr(
                    main_grp + '.' + ATTRIBUTE_IDENTIFIER_NAME,
                    str(object),
                    type='string',
                )
                ...
                for i, frame in enumerate(frames_list):
                    cmds.setKeyframe(
                        main_grp, at=SCREEN_Z_DEPTH_ATTR_NAME, t=frame, v=dlist[i]
                    )
                ...
                con = cmds.parentConstraint(getViewportCamera(), screenz_master_grp)

                fastbake_lib.bake_attributes(
                    [screenz_master_grp], [], start_frame, end_frame, False
                )
                cmds.delete(con)
                # Create screen Z-depth connections
                cmds.connectAttr(
                    main_grp + '.' + SCREEN_Z_DEPTH_ATTR_NAME,
                ...
                screen_xy_master_grp = cmds.group(
                    em=True, n=name + SCREEN_XY_MASTER_NAME
                )
                ...
                fastbake_lib.bake_attributes(
                    [screen_xy_master_grp], [], start_frame, end_frame, False
                )
                cmds.delete(con)
                ...
                fastbake_lib.bake_attributes(
                    [main_grp], ['screenX', 'screenY'], start_frame, end_frame, False
                )
                fastbake_lib.bake_attributes(
                    [screenz_master_grp],
    ...
    def checkNameExists(self, rigName, name, rigs_list):
        ...
        if len(sel) == 0:
            _display_warning_ui(
                'atleast one Transform ' 'node type object must be selected.'
            )
        ...
        if same_name_used is True:
            _display_warning_ui(
                'same name exists already ' 'please type different name.'
            )
        return valid_name_list, same_name_used
    ...
    def createRigBtn(self):
        ...
        for sel_item, name_item in zip(sel, names):
            ...
            if has_constraints is False:
                ...
                _display_warning_ui('selected object(s) already have constraints.')
                return

        for sel_item, name_item, depth_list in node_screen_depths:
            self.createRig(cam_tfm, sel_item, depth_list, name_item, RIG_SUFFIX_NAME)
        return
        ...
        with ctx:
            ...
            for sel_item, name_item in zip(sel, names):
                ...
                if do_camera_space:
                    # Camera space bake
                    self.createRig(
                        cam_tfm, sel_item, dlist, name_item, FREEZE_RIG_SUFFIX_NAME
                    )
                    cmds.cutKey(
                        name_item + FREEZE_RIG_SUFFIX_NAME,
                        clear=True,
                    ...
                    cmds.delete(con)
                    fastbake_lib.bake_attributes(
                        [temp_grp], [], start_frame, end_frame, False
                    )
                    self.createRig(
                        cam_tfm, sel_item, dlist, name_item, FREEZE_RIG_SUFFIX_NAME
                    )
                ...
                for sel_item in selected_items:
                    ...
                    for j in children:
                        ...
                        if bake_options == 'full_bake':
                            ...
                            fastbake_lib.bake_attributes(
                                [object], [], start_frame, end_frame, False
                            )
                        if bake_options == 'smart_bake':
                            ...
                            keys_list = cmds.keyframe(
                                rigName + '.' + SCREEN_Z_DEPTH_ATTR_NAME, query=True
                            )
                            if keys_list:
                                for frame in keys_list:
                                    oma.MAnimControl.setCurrentTime(om.MTime(frame))
    ...
    def deleteRigBtn(self):
        ...
        for sel_item in selected_items:
            ...
            for j in children:
                ...
                nodes_to_delete.append(name + SCREEN_Z_MASTER_NAME)
                nodes_to_delete.append(name + SCREEN_XY_MASTER_NAME)
    ...
    def match(self):
        ...
        if len(selected_items) < 2:
            _display_warning_ui('at least two rigs must' ' be selected from Rigs list.')
            return

        # Get parent
        selected_parent = selected_items[0]
        parent_main = selected_parent.text(0)
        parent_base_name = parent_main.split(RIG_SUFFIX_NAME)[0]
        parent = parent_base_name + SCREEN_Z_MASTER_NAME
        ...
        try:
            ...
            for selected_child in selected_children:
                ...
                child_base_name = child_main.split(RIG_SUFFIX_NAME)[0]
                child = child_base_name + SCREEN_Z_MASTER_NAME
                self.lockUnlockAttributes(child, lock=False)
                attr_list = TRANSFORM_ATTR_LIST
                for attr in attr_list:
                    plug = child + '.' + attr
                    cmd = 'source channelBoxCommand; ' 'CBdeleteConnection "%s";'
                    mel.eval(cmd % plug)
                ...
                cmds.setAttr(child_main + '.' + SCREEN_Z_DEPTH_ATTR_NAME, child_scale)
