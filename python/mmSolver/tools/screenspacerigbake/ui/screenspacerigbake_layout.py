...
def set_z_depth_keyframes(node, start_frame, end_frame, values,
    ...
    for i, frame in enumerate(frames):
        ...
        cmds.setKeyframe(
            node,
            attribute=const.SCREEN_Z_DEPTH_ATTR_NAME,
            value=values[i],
            **kwargs)
    return
...
class ScreenSpaceRigBakeLayout(QtWidgets.QWidget, ui_layout.Ui_Form):
    def __init__(self, parent=None, *args, **kwargs):
        ...
        self.animlayer_action = QtWidgets.QAction(
            'Add to AnimLayer', self)
        ...
        self.camera_space_action = QtWidgets.QAction(
            'Freeze in Camera Space', self)
        ...
        self.world_space_action = QtWidgets.QAction(
            'Freeze in World Space', self)
    ...
    def create_connections(self):
        ...
        self.createFreezeRigButton.clicked.connect(
            self.create_freeze_rig_clicked)
    ...
    def create_screen_space_rig_clicked(self):
        ...
        check_name = self.check_name_exists(const.RIG_SUFFIX_NAME, name,
                                            rigsList)
    ...
    def match(self):
        ...
        try:
            ...
            for selected_child in selected_children:
                ...
                cmds.cutKey(child_main, cl=True,
                            at=const.SCREEN_Z_DEPTH_ATTR_NAME)
                child_scale = cmds.getAttr(child + '.sx')
                cmds.setAttr(child_main + '.' + const.SCREEN_Z_DEPTH_ATTR_NAME,
                             child_scale)
