...
#
"""
Window for the Set Camera Origin Frame tool.
...
import mmSolver.tools.setcameraoriginframe.ui.originframe_layout as originframe_layout
...
def _open_help():
    ...
    page = 'tools_cameratools.html#set-camera-origin-frame'
    helputils.open_help_in_browser(page=page, help_source=src)
    return


class OriginFrameWindow(BaseWindow):
    name = 'SetCameraOriginFrameWindow'

    def __init__(self, parent=None, name=None):
        super(OriginFrameWindow, self).__init__(parent, name=name)
        self.setupUi(self)
        self.addSubForm(originframe_layout.OriginFrameLayout)
...
def main(show=True, auto_raise=True, delete=False):
    """
    Open the Set Camera Origin Frame UI window.
    ...
    :returns: A new origin frame window, or None if the window cannot be
              opened.
    :rtype: OriginFrameWindow or None.
    """
    win = OriginFrameWindow.open_window(show=show, auto_raise=auto_raise, delete=delete)
    return win
