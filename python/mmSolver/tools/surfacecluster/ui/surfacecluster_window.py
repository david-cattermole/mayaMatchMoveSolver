...
#
"""
Window for the Surface Cluster tool.
...
import mmSolver.tools.surfacecluster.ui.surfacecluster_layout as surfacecluster_layout
...
def _open_help():
    ...
    page = 'tools_generaltools.html#surface-cluster'
    helputils.open_help_in_browser(page=page, help_source=src)
    return


class SurfaceClusterWindow(BaseWindow):
    name = 'SurfaceClusterWindow'

    def __init__(self, parent=None, name=None):
        super(SurfaceClusterWindow, self).__init__(parent, name=name)
        self.setupUi(self)
...
def main(show=True, auto_raise=True, delete=False):
    """
    Open the Surface Cluster UI window.
    ...
    :returns: A new surface cluster window, or None if the window cannot be
              opened.
    :rtype: SurfaceClusterWindow or None.
    """
    win = SurfaceClusterWindow.open_window(
